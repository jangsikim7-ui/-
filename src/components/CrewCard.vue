<template>
  <div class="card" :style="{ '--c': crew.color }">
    ...
    <!-- 크루 매출풍 (별풍선 모드 + 수장 등록된 경우만) -->
    <div v-if="mode === 'balloon' && crew.master_soop_id" class="master-revenue-bar">
      <div class="master-left">
        <span class="master-crown">👑</span>
        <span class="master-label">크루 매출풍</span>
      </div>
      <span class="master-val">{{ fmt(crew.master_balloons) }}</span>
    </div>
...
async function toggleMember(m) {
  if (selectedMember.value?.id === m.id) { selectedMember.value = null; return }
  ...
  } catch(e) { fanError.value = '불러오기 실패: ' + e.message }
...
function tierClass(n) {
  if (props.mode === 'viewer') {
    if (n>=500000) return 't1'; if (n>=400000) return 't2'; if (n>=300000) return 't3'; if (n>=150000) return 't4'; return 't5'
  }
  if (n>=1300000) return 't1'; if (n>=1000000) return 't2'; if (n>=700000) return 't3'; if (n>=400000) return 't4'; return 't5'
}
function tierColor(n) {
  if (props.mode === 'viewer') {
    if (n>=500000) return '#ff4d7d'; if (n>=400000) return '#f5a623'; if (n>=300000) return '#4cd964'; if (n>=150000) return '#4a9eff'; return '#666680'
  }
...
function onImgError(e, soopId) {
  ...
  else { e.target.style.display = 'none'; const ph = e.target.nextElementSibling; if (ph) ph.style.display = 'flex' }
}
</script>

<style scoped>
.card {
  background: var(--bg3); border-radius: 16px; border: 1px solid var(--border);
  overflow: hidden; display: flex; flex-direction: column;
  box-shadow: 0 8px 32px rgba(0,0,0,0.18); transition: transform 0.2s, box-shadow 0.2s;
  max-width: 290px; position: relative;
}
.card:hover { transform: translateY(-4px); box-shadow: 0 16px 40px rgba(0,0,0,0.25), 0 0 20px color-mix(in srgb, var(--c) 12%, transparent); }

.card-header {
  display: flex; align-items: center; gap: 8px; padding: 12px 12px 10px;
  border-bottom: 1px solid var(--border);
  background: linear-gradient(135deg, color-mix(in srgb, var(--c) 60%, var(--bg4)) 0%, color-mix(in srgb, var(--c) 28%, var(--bg3)) 50%, var(--bg3) 100%);
  position: relative; overflow: hidden;
}
[data-theme="light"] .card-header { background: linear-gradient(135deg, color-mix(in srgb, var(--c) 55%, #fff) 0%, color-mix(in srgb, var(--c) 22%, #fff) 50%, #fff 100%); }
.card-header::before { content: ''; position: absolute; top: -30px; left: -20px; width: 110px; height: 110px; background: radial-gradient(circle, color-mix(in srgb, var(--c) 35%, transparent), transparent 70%); pointer-events: none; }
...
.master-revenue-bar {
  ...
  border-bottom: 1px solid rgba(255, 190, 0, 0.22);
}
[data-theme="light"] .master-revenue-bar { background: rgba(255,170,0,0.08); border-bottom-color: rgba(255,170,0,0.2); }
.master-left { display: flex; align-items: center; gap: 5px; }
.master-crown { font-size: 13px; }
...
[data-theme="light"] .master-val { color: #b06000; }
[data-theme="light"] .master-label { color: #7a4a00; }
...
.member-row { display: flex; align-items: center; gap: 7px; padding: 7px 12px 12px; border-top: 1px solid var(--border2); position: relative; transition: background 0.12s; }
...
.member-row.top1 { background: linear-gradient(135deg, color-mix(in srgb, var(--c) 25%, var(--member-bg)) 0%, color-mix(in srgb, var(--c) 12%, var(--member-bg)) 50%, color-mix(in srgb, var(--c) 25%, var(--member-bg)) 100%); background-size: 200% 100%; animation: top1Shimmer 4s linear infinite; border-left: 4px solid var(--c); padding: 16px 12px 14px 10px; flex-wrap: wrap; gap: 0; margin-top: 8px; position: relative; }
.member-row.top1::after { content: '👑 1ST'; position: absolute; top: -9px; left: 12px; background: linear-gradient(135deg, var(--c), color-mix(in srgb, var(--c) 70%, #fff)); color: #fff; font-size: 9px; font-weight: 900; padding: 3px 9px; border-radius: 10px; letter-spacing: 0.5px; box-shadow: 0 2px 8px color-mix(in srgb, var(--c) 50%, transparent); text-shadow: 0 1px 2px rgba(0,0,0,0.3); z-index: 2; }
@keyframes top1Shimmer { 0% { background-position: 0% 0; } 100% { background-position: 200% 0; } }
[data-theme="light"] .member-row.top1 { background: linear-gradient(135deg, color-mix(in srgb, var(--c) 22%, #fff) 0%, color-mix(in srgb, var(--c) 8%, #fff) 50%, color-mix(in srgb, var(--c) 22%, #fff) 100%); background-size: 200% 100%; animation: top1Shimmer 4s linear infinite; }
.member-row.top1 .rank-badge { display: none; }
.member-row.top1 .profile-wrap { width: 38px; height: 38px; }
.member-row.top1 .profile-img { width: 38px; height: 38px; border: 2px solid var(--c); box-shadow: 0 0 12px color-mix(in srgb, var(--c) 60%, transparent); }
...
.member-row.top1 .mval { font-size: 17px; font-weight: 900; text-shadow: 0 0 8px color-mix(in srgb, var(--c) 30%, transparent); }
[data-theme="light"] .member-row.top1 .mval { text-shadow: none; }
.top1-crown { display: none; }
.expand-arrow { font-size: 14px; color: var(--text4); transition: transform 0.2s; flex-shrink: 0; }
...
.fan-panel { background: color-mix(in srgb, var(--c) 5%, var(--bg3)); border-top: 1px solid color-mix(in srgb, var(--c) 20%, var(--border)); border-bottom: 1px solid color-mix(in srgb, var(--c) 20%, var(--border)); padding: 8px 0 10px; animation: fanSlide 0.2s ease; }
@keyframes fanSlide { from { opacity: 0; transform: translateY(-6px) } to { opacity: 1; transform: translateY(0) } }
.fan-title { font-size: 10px; font-weight: 700; color: var(--c); padding: 0 12px 6px; text-transform: uppercase; letter-spacing: 0.5px; }
.fan-loading { display: flex; align-items: center; gap: 6px; padding: 16px 12px; font-size: 12px; color: var(--text3); }
.spin { animation: spin 1s linear infinite; display: inline-block; }
@keyframes spin { to { transform: rotate(360deg) } }
.fan-error { padding: 12px; color: #ff4d4d; font-size: 12px; }
.fan-list-head { grid-template-columns: 28px 1fr 70px; padding: 0 12px 4px; font-size: 9px; font-weight: 600; color: var(--text4); text-transform: uppercase; letter-spacing: 0.3px; border-bottom: 1px solid var(--border2); margin-bottom: 2px; }
.fan-row { display: grid; grid-template-columns: 28px 1fr 70px; align-items: center; padding: 5px 12px; gap: 4px; transition: background 0.1s; }
...
[data-theme="light"] .t1{color:#d6004a}[data-theme="light"] .t2{color:#b87000}[data-theme="light"] .t3{color:#1e9e3a}[data-theme="light"] .t4{color:#1565c0}[data-theme="light"] .t5{color:#1a1a2e}
...
.card-foot { display: flex; align-items: stretch; border-top: 1px solid var(--border); background: var(--foot-bg); }
